...
namespace RabbitMQTopic
{
    ...
    public class Consumer
    {
        ...
        private readonly ConcurrentDictionary<string, ConcurrentDictionary<byte, Tuple<IModel, Thread>>> _globalChannels
            = new ConcurrentDictionary<string, ConcurrentDictionary<byte, Tuple<IModel, Thread>>>();

        private readonly ConcurrentDictionary<string, ConcurrentDictionary<byte, EventingBasicConsumer>>
            _globalConsumers = new ConcurrentDictionary<string, ConcurrentDictionary<byte, EventingBasicConsumer>>();
                    ...
                    if (_mode == ConsumeMode.Push)
                    {
                        foreach (var topic in _globalConsumers.Keys)
                        {
                            ...
                            foreach (var queueIndex in consumers.Keys)
                            {
                                ...
                                if (channel.IsOpen)
                                {
                                    channel.Close(RabbitMQConstants.ConnectionForced,
                                        $"\"{topic}-{queueIndex}\"'s normal channel disposed");
                                }
                            }
                ...
                for (byte queueIndex = 0; queueIndex < queueCount; queueIndex++)
                {
                    ...
                    try
                    {
                        consumer.Received += (sender, e) =>
                        {
                            ...
                            while (consumers.All(w => w.Value.Model != currentChannel))
                            {
                                Thread.Sleep(1000);
                            }
                        ...
                        channel.BasicConsume(GetQueue(topic, queueIndex), false,
